...
- 2026-09-05 — **Prospect Finder de-scoped from cybersecurity → any industry/title/location, with typo correction + strict relevance.** Root causes of "always searches cybersecurity / wrong location": (1) `apollo-import-service.upsertCompany` hardcoded `industry: p.industry ?? "cybersecurity"` → every imported company mislabeled cyber whenever Apollo's api_search redacts the org industry (this plan always does) — now `"Unknown"`; (2) no typo/format correction → `US`/`Canda`/`Cybersecuirty`/bare `Kabul` silently failed Apollo's literal filters; (3) no client-side relevance guard. Fixes: NEW `normalizeProspectQuery` in `ai-service.ts` (tool `ai-parse-search` = 0.02; workflowKey `prospect_search` → unknown key falls through to global mode; `confidence:undefined` so Hybrid doesn't block) corrects titles/industry/location spelling, expands abbreviations, adds country to bare cities, stays within intended industry/region — **best-effort with silent fallback** (returns null on Human/dummy/wallet-block/unparseable, search never breaks). `apollo-service.searchPeople` now: deterministic baseline normalize (always-on alias map US→United States etc.) → AI correct → build body → **strict location post-filter** `filterByRequestedLocations` (drops off-region + blank-location rows; redaction safeguard: if EVERY row is blank-location, trust Apollo's server-side `person_locations` filter instead of returning nothing). Response gained `appliedFilters`/`queryNotice`/`droppedForLocation` (hand-written `/apollo/search` hook — no codegen). Added 4 non-cyber sample seeds (Banking/Blockchain/Healthcare/SaaS · Kabul/Canada/US/India). Frontend: generic copy/placeholders, "Interpreted as:" note, off-location counts. **Deployed + verified on :3200** (new code in both images). ⚠️ Runtime: Apollo LIVE but `AI_DUMMY_MODE=true` → AI correction dormant (deterministic layer + strict filter + cyber-default fix still active); arbitrary-typo correction needs `AI_DUMMY_MODE=false` (OpenAI key already set). 0 new tsc errors; both build green. See [[apollo-integration]], [[docker-rebuild-verify]].
- 2026-09-05 — **Fixed "already in pipeline" blocking re-import of a cleared pipeline.** `importProspects` deduped on `contacts.externalCrmId`, but `DELETE /leads/:id` removes only the LEAD — the contact (and company) survive. DB confirmed the leak: **307 orphaned Apollo contacts vs 6 leads**, so re-search skipped ~301 people as `already_imported` against ghosts of deleted prospects ("does not show all results / already in pipeline with nothing in it"). Fix: dedup now checks for a live **lead** (`leadsTable.externalCrmId`) = actual pipeline presence; and re-import **reuses** any lingering contact (by `externalCrmId`, re-points its `companyId`, preserves revealed email/phone — no dupes, no re-pay) instead of inserting a new one. `checkDuplicatesOnCreate("contact")` now runs only for freshly-created contacts. No frontend pre-filter involved. Deployed + verified on :3200 (api bundle has `existingLead`/`existingContact`). NOTE: the 307 orphan contacts/278 companies remain (harmless — reused on re-import, not shown in pipeline); a purge is available if a truly clean slate is wanted. 0 new tsc errors. See [[apollo-integration]].
- 2026-09-05 — **Purged hallucinated/static data from Prospect Finder + CRM cards (dummy-mode data masquerading as real).** Two user-visible fakes, both from `AI_DUMMY_MODE=true`: (1) the detail dialog subtitle was `selectedLead.title ?? selectedLead.bestAngle ?? "Prospect"` — leads have `contactTitle` not `title`, so it ALWAYS fell through to `bestAngle`, which in dummy mode is `DUMMY_AI_RESPONSES["ai-enrich-lead"]` (fabricated "Austin TX / CrowdStrike / $18M Series B / SOC 2", sliced to 500 chars). Fixed subtitle → `contactTitle || humanizeSeniority(seniority) || "Prospect"`. (2) Every company showed industry "cybersecurity" — hardcoded default in `upsertCompany` (fixed earlier → "Unknown") AND `leads.ts` manual Add-Lead (line 147, → "Unknown"); enrichment never filled the real industry (`orgToCompanyValues` dropped it). Fixes: `orgToCompanyValues` now emits `industry`; `companyRichCols` selects it; `fillEmptyCompanyById` treats the "Unknown" placeholder as empty so live enrichment (bulk_match/PDL) overwrites it; `ProspectCard`/`CompanyFacts` hide "Unknown" (never render a placeholder as a fact). Also made the dummy `ai-enrich-lead` (→ honest note, no fabricated facts) and `ai-score-company` (→ SCORE/TIER/REASONING honest text, was a JSON blob landing in `leads.notes` and surfacing on the CRM card via `bestAngle ?? painPoints ?? notes`). **DB cleanup (pgdata):** 273 companies `cybersecurity`→`Unknown` (0 had provider-sourced industry, so all were wrong guesses); nulled 26 leads' dummy `bestAngle` and 26 dummy `notes` (markers must match the FIRST 500 chars — `companySize`/`CrowdStrike`, not `marketingMaturity` which is truncated off). Deployed to :3200. 0 new tsc errors. NOTE: real per-prospect industry only populates once real AI/enrichment runs; until then industry is honestly blank. See [[apollo-integration]], [[docker-rebuild-verify]].
- 2026-09-05 — **"Enrich once" — never re-enrich a prospect that already has a revealed email.** Enforced at both layers. Frontend (`outreach.tsx`) — ALL enrich entry points guarded (there are only two: the bulk header button + the detail dialog; CRM has no enrich action): `handleBulkEnrich` skips already-`hasEmail` leads (toasts "N already enriched, skipped"); the bulk "Enrich N" button now counts only `selectedNeedingEnrich` (selected leads without an email) and is disabled + relabeled "Enriched" (CheckCircle) when that's 0; `handleEnrichLead` early-returns; the dialog Enrich button is disabled + relabeled "Enriched" when `hasEmail(selectedLead)`; `ProspectCard` shows an "Enriched" badge when `contactEmail` present and not already Verified. Backend: NEW `partitionEnriched(contactIds)` in `apollo-import-service` (uses `normalizeRevealedEmail`, so the `email_not_unlocked` sentinel doesn't count as enriched); `/apollo/enrich` partitions first, early-returns `{...zeros, alreadyEnriched}` when none remain, and runs the WHOLE cascade (website pre + Apollo bulk_match + PDL + website post) on `toEnrich` only — so an enriched contact incurs no re-charge and no redundant provider calls from any caller. (`enrichContacts` already skipped email-having contacts for the Apollo step; this extends the guarantee to website/PDL and the route.) Deployed + verified on :3200 (api bundle has `partitionEnriched`; web has the guard). 0 new tsc errors. See [[apollo-integration]].
- 2026-09-09 — **Social Command Phase 0 (foundations) built + deployed.** Kicks off the Outreach → unified social inbox (people who DM/comment/like/mention across LinkedIn/Meta/WhatsApp/website → one threaded history → deals). **Decisions locked** (user-selected via AskUserQuestion): multi-tenant schema now / single UI first · **Website + Lead Ads → WhatsApp** first · compliant **Meta core now + LinkedIn opt-in "Pro" via Unipile later** (personal LinkedIn DMs/engagement have NO compliant API — gray-market only, ban risk) · **WhatsApp direct Meta Cloud API**. Added 5 tenant-scoped tables (`social_accounts`, `channel_identities`, `conversations`, `messages`, `social_interactions`) via an idempotent CREATE-TABLE script ([lib/db/src/migrate-social-command.ts](lib/db/src/migrate-social-command.ts) + `migrate:social`; `drizzle-kit push` still blocked) — applied to `pgdata`, 5 tables verified. New services: `identity-service` (resolve platform id → contact: known-identity → exact email/phone auto-merge → new+dedup-suggest, reuses `checkDuplicatesOnCreate`) + `social-ingest-service` (canonical `InboundMessageEvent`/`InboundInteractionEvent` → threaded conversations/messages, idempotent dedup on external ids, emits `social.message.received`/`social.interaction.received` + ws `social_message`/`social_interaction`). Read/verify routes `GET/PATCH /api/social/*` (conversations, thread+messages, interactions, accounts, contact identities, stats) live at :3200 with honest empty states; **OpenAPI codegen deferred to Phase 5 UI** (no consumer yet + orval churn risk). Also fixed a **pre-existing Dockerfile breakage unrelated to this work**: dropped the invalid `pnpm install --fetch-retries` flag (newer pnpm removed it → was failing ALL image rebuilds; value still set in the image `.npmrc`). 0 new tsc errors (78 pre-existing baseline unchanged); lib/db + api build green. Feasibility study + full phase plan in [Social-Command-Feasibility-and-Plan.md](Social-Command-Feasibility-and-Plan.md). See [[social-command-initiative]].
- 2026-09-09 — **Social Command Phase 1 (Website + Lead Ads) + Phase 2 WhatsApp adapters built (env-gated, inert until keys).** The provider translation layer that feeds Phase 0's `ingestMessage`/`ingestInteraction`. NEW [services/social-providers-service.ts](artifacts/api-server/src/services/social-providers-service.ts): env config readers (`getMetaConfig`/`getWhatsAppConfig`/`getWebsiteFormSecret`, all `process.env`-based like `getApolloWebhookConfig`) + `getSocialProvidersStatus()` (booleans only, never secrets); **website form** → thread + pipeline lead; **Meta Lead Ads** (`leadgen` webhook → Graph `GET /{leadgen_id}` with Page token → parse `field_data` → thread + lead); **WhatsApp Cloud** inbound parser (text/media/interactive/location + delivery-status roll-up, dedup by `wamid`) + `sendWhatsAppMessage` outbound + `sendConversationReply` (WhatsApp-only this phase; website/lead-ad/Messenger return an honest "not send-capable yet"). Each inbound hand-raise creates a `lead` linked to the identity-resolved contact + sets `conversations.convertedLeadId` + emits `lead.created` (flows into existing pipeline). NEW PUBLIC [routes/social-webhooks.ts](artifacts/api-server/src/routes/social-webhooks.ts) mounted BEFORE `requireAuth` (Apollo-webhook precedent): `GET/POST /api/webhooks/{meta,whatsapp}` (GET = hub.challenge handshake; POST = `X-Hub-Signature-256` HMAC over raw body) + `POST /api/webhooks/website-form` (optional `?secret=`). Added raw-body capture to [app.ts](artifacts/api-server/src/app.ts) `express.json({verify})` (re-stringify would break Meta's signature). Authed [routes/social.ts](artifacts/api-server/src/routes/social.ts) gained `GET /social/providers` (config status) + `POST /social/conversations/:id/reply`. **Wired 10 env passthroughs into [docker-compose.yml](docker-compose.yml) `api.environment` + [.env.docker.example](.env.docker.example)** — WITHOUT this the user's `.env` edits never reach the container (the 2026-09-01 invisible-change trap). No-secret → signature check allowed (matches integration-hub) so ngrok testing works before wiring secrets. **NO schema change, NO OpenAPI codegen** (hand-written routes; typed hooks still deferred to Phase 5 UI). 0 new tsc errors (78 baseline unchanged); api builds green. USER GO-LIVE: paste Meta/WhatsApp creds into `.env`, point webhooks at a PUBLIC URL (Meta can't reach localhost), `docker compose up -d --build`. See [[social-command-initiative]].
- 2026-09-10 — **Social Command Phase 5 (unified inbox UI) built + deployed to :3200.** Rebuilt the Outreach → Social Command tab from a flat `communications` reader into a real threaded inbox over the Phase-0 `/api/social/*` layer. NEW hand-written hooks in [use-api.ts](artifacts/pmg-os/src/hooks/use-api.ts) (`useSocialConversations`/`useSocialConversation`/`useSocialInteractions`/`useSocialProviders`/`useSocialAccounts`/`useSocialStats` + `usePatchConversation`/`useReplyConversation`/`usePatchInteraction`) with typed `Social*` interfaces. **DECISION — hand-written hooks, NOT OpenAPI codegen** (deviates from the Phase-5 plan/[[social-command-initiative]] "codegen happens here"): the routes return hand-shaped SQL joins (contactName, nested messages) that don't map to generated CRUD, and orval churn is fragile (pinned 8.5.3) — same precedent as `/apollo/*` + `/social/providers`. Rewrote `SocialCommand` in [outreach.tsx](artifacts/pmg-os/src/pages/outreach.tsx): honest connection strip from real `/social/providers` booleans (Website Forms **Live** · Lead Ads/WhatsApp **Not connected** until keys · Messenger/IG/LinkedIn **Planned**); master-detail inbox (status+channel filters, unread badges, intent chips, per-person thread with inbound/outbound bubbles + per-message source chip + `ModeBadge` on sent replies, `human_controlled`→`human` normalized); reply composer gated to WhatsApp send-capability (`primaryChannel==="whatsapp" && outboundConfigured`), honest disabled note otherwise + surfaces the route's 422; mark-read on open, snooze/close/reopen; Move-to-CRM (respects existing `convertedLeadId`); Interactions sub-view with reviewed/CRM triage (`usePatchInteraction`). Extended `socialChannelIcon` (whatsapp/messenger/lead); dropped now-unused `useIntegrationStatus` import. **Verified live on :3200**: rebuilt `web` image (compose recovered from a transient `pmg-os-postgres` name conflict — all 3 containers up fresh on `greenfield-ai-os_default`); served bundle has the new strings, old "Route replies into this inbox" GONE; `login=200`, `/api/social/{providers,stats,conversations}` all 200 with honest empty states (0 convos, Website Forms configured, Meta/WA not). 0 new tsc errors (pmg-os baseline still the lone pre-existing `use-websocket.ts` error); vite build green. USER GO-LIVE: paste Meta/WhatsApp creds + public webhook to see real threads. Phase 4 (AI intent→deal scoring + tri-mode drafting) is the next layer on top. See [[social-command-initiative]], [[docker-rebuild-verify]].
- 2026-09-10 — **Social Command Phase 4 (intent → deal) built + deployed to :3200.** NEW [services/social-intent-service.ts](artifacts/api-server/src/services/social-intent-service.ts) subscribes to the Phase-0 `social.message.received` / `social.interaction.received` events and turns inbound engagement into scored, tri-mode-governed action. **(1) Intent scoring** — a deterministic keyword heuristic runs ALWAYS (free, dummy-safe, never throws out of a handler); when real AI is on AND the mode gate allows, it's REFINED by a wallet-billed classifier (`ai-score-social-intent`, workflow `social_intent`=ai_autonomous). Score+sentiment persist on the message/interaction and roll up to `conversations.intentScore` (max) → the Phase-5 inbox intent chips reflect real warmth. **(2) High-intent → lead** — a conversation scoring ≥70 not already converted is pushed to the pipeline via `executeOrQueue` (actionType `social_convert`, **UNREGISTERED** workflow key → follows GLOBAL mode: Auto creates the lead + fires `lead.created` immediately; Hybrid/Human queue a pending-action approval). Idempotent on `convertedLeadId` (no dup with the provider-layer `createLeadFromSocial`). **(3) AI reply draft** — a warm inbound (≥50) gets an AI draft (`ai-draft-social-reply`) stored on `conversations.metadata.suggestedReply` (surfaced in the inbox thread with a "Use draft" button) AND routed through `executeOrQueue` (actionType `social_reply`, follows global: Auto sends via WhatsApp when send-capable, Hybrid/Human queue). Both action executors registered so approvals resolve the send/convert. **Mode-safe throughout** (prospect-normalizer pattern): both AI calls pre-check `shouldAiAct` (no "Human Action Required" spam) and are gated on `!isDummyMode()` — so in the current dummy runtime ONLY the heuristic + high-intent auto-convert run; NO fabricated drafts. Wiring: +2 `TOOL_COSTS` (`ai-score-social-intent` 0.02, `ai-draft-social-reply` 0.05); `social_intent` added to `DEFAULT_WORKFLOW_MODES`; honest dummy responses added; `initSocialIntent()` in [index.ts](artifacts/api-server/src/index.ts) beside `initLeadRouter()`; 2 new `WsEventType`s; `metadata` added to the `/social/conversations/:id` select + `SocialConversationDetail` type + thread footer. **Verified live on :3200**: api log shows `[SocialIntent] Initialized`; `/api/ai-mode/workflows` exposes `social_intent`=ai_autonomous; web bundle has the "AI suggested reply" affordance; login + social APIs 200. 0 new tsc errors (api-server back to **78 baseline**; pmg-os the lone pre-existing `use-websocket.ts`); both build green. Reply auto-send + AI drafts activate when `AI_DUMMY_MODE=false` + a WhatsApp token is wired. See [[social-command-initiative]], [[docker-rebuild-verify]].
