...
export interface FinanceOverview {
  kpis: { mrr: number; outstanding: number; collectedMtd: number; activeContracts: number };
  invoices: Array<{ id: number; invoiceNumber: string; client: string; type: string; status: string; amount: number; totalAmount: number; dueDate: string | null; paidAt: string | null; package: string }>;
  revenueByClient: Array<{ client: string; revenue: number; cost: number; package: string; months: number }>;
  contracts: Array<{ id: number; client: string; package: string; monthlyValue: number; status: string; effectiveDate: string | null; expirationDate: string | null; renewalDate: string | null }>;
  expenses: Array<{ category: string; items: Array<{ name: string; cost: number }>; total: number }>;
  pnl: { mrr: number; totalExpenses: number; profit: number; margin: number };
  scenarios: Array<{ label: string; newMRR: number; annual: number }>;
}
...
export interface ProductionOverview {
  leads: Array<{ id: number; company: string; contact: string; email: string; phone: string; score: number; pain: string; approach: string; status: string }>;
  assets: Array<{ id: number; name: string; type: string; format: string; date: string | null; status: string; score: number | null }>;
  campaigns: Array<{ id: number; client: string; name: string; channel: string; status: string; funnel: string; metrics: { visitors: number; leads: number; meetings: number; clients: number }; budget: number; spent: number }>;
  report: { title: string; summary: string | null; period: string | null; data: any } | null;
  quality: Array<{ id: number; name: string; type: string; score: string; issues: string[]; details: string }>;
}

export function useProductionOverview() {
  return useQuery({
    queryKey: ["/api/production/overview"],
    queryFn: () => apiFetch<ProductionOverview>("/production/overview"),
  });
}
...
export function useAiOutputs(kind: string, opts?: { domain?: string; entityType?: string; entityId?: number; limit?: number }) {
  ...
  return useQuery({
    queryKey: ["/api/ai-outputs", kind, opts],
    queryFn: () => apiFetch<AiOutput[]>(`/ai-outputs?${params.toString()}`),
  });
}
...
export function useSaveAiOutput() {
  ...
  return useMutation({
    mutationFn: (body: { domain: string; kind: string; title: string; summary?: string; data?: any; entityType?: string; entityId?: number; generatedBy?: string }) =>
      apiFetch<AiOutput>("/ai-outputs", { method: "POST", body: JSON.stringify(body) }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["/api/ai-outputs"] }),
  });
}

export interface AdminOverview {
  kpis: { activeTasks: number; overdueTasks: number; docs: number; teamSize: number };
  team: Array<{ name: string; role: string; tasks: number; completed: number; overdue: number }>;
  tasks: Array<{ id: number; task: string; assignee: string; due: string; priority: string; source: string; status: string }>;
  documents: Array<{ id: number; title: string; category: string; updated: string; autoGenerated: boolean }>;
...
export function useCompleteTask() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (id: number) => apiFetch(`/tasks/${id}`, { method: "PATCH", body: JSON.stringify({ status: "completed" }) }),
...
// Reveal contact data for imported contacts. Live = Apollo bulk_match (~1 credit
// per email); fixture = labeled sample data. `revealPhone` also captures any phone
// Apollo returns synchronously (already-known numbers; new mobiles are webhook-only).
...
export function useApolloEnrich() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ contactIds, revealPhone }: { contactIds: number[]; revealPhone?: boolean }) =>
      apiFetch<ApolloEnrichResult>("/apollo/enrich", {
        method: "POST",
        body: JSON.stringify({ contactIds, revealPhone: revealPhone ?? false }),
      }),
    onSuccess: () => {
...
export function useOptOutList() {
  return useQuery({
    queryKey: ["opt-out"],
    queryFn: () => apiFetch<{ contacts: string[]; total: number }>("/opt-out"),
  });
}
...
export function useAddOptOut() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (email: string) => apiFetch<{ success: boolean; email: string }>("/opt-out", { method: "POST", body: JSON.stringify({ email }) }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["opt-out"] }),
  });
}
...
export function useRemoveOptOut() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (email: string) => apiFetch<{ success: boolean; email: string }>("/opt-out", { method: "DELETE", body: JSON.stringify({ email }) }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["opt-out"] }),
  });
}
...
export function useAiQualifyLead() {
  return useMutation({
    mutationFn: (data: { leadData?: any; companyName?: string; problemStatement?: string; contactName?: string; source?: string }) =>
...
export function useAiPrepareCall() {
  return useMutation({
    mutationFn: (data: { prospectName?: string; companyName?: string; callObjective?: string; priorContext?: string; dealStage?: string; serviceType?: string }) =>
...
export function useAiCreateProposal() {
  return useMutation({
    mutationFn: (data: { prospectName?: string; companyName?: string; discoveryFindings?: string; budget?: number; serviceType?: string; dealValue?: number }) =>
...
export function useAiCoachingCards() {
  return useMutation({
    mutationFn: (data: { companyName?: string; dealValue?: number; serviceType?: string; stage?: string }) =>
      apiFetch<{ cards: Array<{ trigger: string; response: string; category: string }>; raw: string }>("/crm/coaching", { method: "POST", body: JSON.stringify(data) }),
  });
}
...
export interface TranscriptAnalysisResult {
  analysis: {
    ...
    keyTopics: string[];
    sentiment: { overall: string; score: number; moments: Array<{ text: string; sentiment: string }> };
...
export function useProcessTranscript() {
  return useMutation({
    mutationFn: (data: { transcript: string; type?: string; contactName?: string; companyName?: string }) =>
      apiFetch<TranscriptAnalysisResult>("/ai/transcript/process", { method: "POST", body: JSON.stringify(data) }),
  });
}
