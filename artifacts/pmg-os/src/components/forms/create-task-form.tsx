...
export function CreateTaskForm({ open, onOpenChange }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await create.mutateAsync({ ...form, dueDate: form.dueDate || undefined });
      ...
      <DialogContent className="sm:max-w-[500px] bg-[hsl(214,65%,6%)] border-white/10">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="space-y-2"><Label className="text-slate-300">Assignee</Label><Input value={form.assignee} onChange={e => setForm(f => ({ ...f, assignee: e.target.value }))} className="bg-white/5 border-white/10 text-white" /></div>
