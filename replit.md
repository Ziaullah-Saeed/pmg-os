...
*   **Multi-Tool Orchestration:** 24 registered tools with sequential chain execution. 7 chain templates: `lead_qualification`, `outreach_pipeline`, `icp_analysis`, `competitor_intel`, `deal_assessment`, `call_analysis` (transcript→sentiment→objections→follow-up), `meeting_setup` (availability→booking). Tri-mode aware chain execution. Routes: `GET /ai/tools`, `GET /ai/chains`, `POST /ai/chain/execute`.
