...
*   **Finance, Legal & Quality (Phase 7):** Comprehensive finance-legal-quality service layer. **Invoice Lifecycle:** Full state machine (draft→sent→viewed→overdue→partially_paid→paid→cancelled→written_off) with transition validation, auto-overdue detection, payment recording with auto-status progression. **Expense Approval Workflow:** Submission creates approval records via approval engine, priority escalation ($5K+ critical, $1K+ high), approve/reject with audit trail and notifications. **Contract AI Review:** AI-powered clause flagging, risk scoring (0-100), missing clause detection, compliance analysis (GDPR, CCPA, SLA, IP, liability), auto-flags contracts for human review when riskScore≥70. AI contract generation from templates. **Quality Checkpoints:** 11 checkpoint rules across CRM/finance/production domains, enforced on entity operations, auto-creates quality_issues on failure. **SOP Enforcement:** Action-to-SOP category mapping (invoice_send, contract_sign, expense_approve, lead_qualify, asset_publish, proposal_send, onboarding_start), compliance checking against active SOPs, AI-powered SOP audit. 10 new finance-legal tools, 3 new chain templates (invoice_lifecycle, contract_review_pipeline, expense_approval_pipeline), 7 workflow modes. **Totals: 44 tools, 12 chains, 50 agents mapped.** Routes: `POST /ai/invoice/transition`, `POST /ai/invoice/record-payment`, `POST /ai/invoice/check-overdue`, `POST /ai/expense/submit`, `POST /ai/expense/review`, `POST /ai/contract/review`, `POST /ai/contract/generate`, `POST /ai/quality/check`, `GET /ai/quality/checkpoints/:entityType`, `POST /ai/sop/check`, `POST /ai/sop/audit`.
*   **Integration Hub (Phase 8):** Unified third-party integration layer. **10 Connectors:** GoHighLevel, HubSpot, Salesforce, LinkedIn, Meta, Google Ads, Apollo, Stripe, Slack, Mailchimp — supporting OAuth2 and API key auth. **OAuth2 Flows:** Authorize URL builder, code exchange, token refresh per-provider. **Public Webhook Receiver:** `/webhooks/inbound/:source` endpoint (no auth) with 8 source handlers (lead_form, landing_page, facebook_lead, linkedin_lead, gohighlevel, hubspot, stripe, custom). Auto-creates leads/contacts/companies from webhook payloads. HMAC-SHA256 signature verification support. **CSV Import:** Multi-entity import (leads, contacts, companies, opportunities) with field mapping, validation, dry-run preview, skip-duplicates. **Sync Engine:** Bidirectional sync with exponential retry (3 attempts: 5s/15s/60s), sync health monitoring, sync log tracking, manual trigger + scheduled sync. Routes: `GET /integration-hub/connectors`, `GET /integration-hub/status`, `POST /integration-hub/oauth/authorize`, `POST /integration-hub/oauth/callback`, `POST /integration-hub/oauth/refresh`, `POST /integration-hub/connect-api-key`, `POST /integration-hub/disconnect`, `GET /integration-hub/sync/logs`, `GET /integration-hub/sync/health`, `POST /integration-hub/sync/trigger`, `POST /integration-hub/sync/schedule`, `POST /integration-hub/sync/stop`, `POST /integration-hub/sync/retry/:logId`, `GET /integration-hub/import/entities`, `POST /integration-hub/import/validate-mapping`, `POST /integration-hub/import/preview`, `POST /integration-hub/import/execute`, `GET /integration-hub/webhook/sources`, `POST /integration-hub/webhook/generate-secret`.
*   **Reporting & Knowledge Memory (Phase 9):** Comprehensive reporting and knowledge auto-population layer. **Scheduled Reports:** 5 cron-based report templates (daily_pipeline, weekly_revenue, weekly_marketing, monthly_executive, quarterly_business) registered as scheduler jobs with AI-powered content generation. Automatic fallback to raw metrics when AI unavailable. **Event-Triggered Reports:** Auto-generates milestone reports on `opportunity.won`, `opportunity.lost`, `invoice.paid`, `lead.converted` events. Reports include AI executive summaries with domain-specific metrics. **Knowledge Auto-Population:** 8 event-to-knowledge mappings auto-create entries from business events (deal wins/losses, lead scoring/enrichment/conversion, contract signing, invoice payments, report generation). All entries auto-embed for semantic search. **Permission-Aware Archive:** Role-based access filtering (super_admin→all levels, admin→internal+confidential+public+client, manager→internal+public+client, user→public only). **Delivery Channels:** Slack (via connected integration with Block Kit formatting) and email delivery for any report. Routes: `GET /reporting/templates`, `GET /reporting/event-triggers`, `POST /reporting/generate`, `POST /reporting/deliver/slack`, `POST /reporting/deliver/email`, `GET /reporting/archive`, `GET /knowledge/event-mappings`.
